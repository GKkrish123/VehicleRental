...
class add_rental_booking_model(BaseModel):
    ...
    rentaldate: Optional[str] = Field(date.today().strftime('%d/%m/%Y'),description="Date of rental booking",example="dd/mm/yyyy")
    returndate: Optional[str] = Field(None,description="Date of return of rental vehicle",example="dd/mm/yyyy")
    ...
    def should_be_valid_vehicletype(cls, v):
        try:
            ...
            if data == None:
                raise ValueError(f'Currently, {v} is not available in our inventory. Please checkout the vehicle availability')
            if data.inventory == 0:
                raise ValueError(f'Currently, {v} cannot be rented as it is already booked')
        except ValueError as e:

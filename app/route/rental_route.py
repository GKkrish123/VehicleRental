...
def get_rental_booking(
    bookingids: List[int] = Query(
        None, description="Enter the ids of rental booking", gt=0
    ),
    ...
    try:
        ...
        rental_booking_details = (
            session.query(Rentalbooking).filter(*booking_filters).all()
        )
...
def add_rental_booking(rental_booking_details: add_rental_booking_model):
    try:
        ...
        vehicle_inventory = (
            ...
            .filter(Vehicle.vehicletype == rental_booking_details["vehicletype"])
            .first()
        )
